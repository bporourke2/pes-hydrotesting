...
class PipelineApp:
    ...
    def generate_plot(self, df, min_test=None, params=None, gauge_lower=None, gauge_upper=None, prepack_time=None, sec=None, vent_time=None):
        fig = go.Figure()
        ...
        # Fill exceeds area (approximated as shaded region)
        exceed_mask = df['Local_P'] > df['SMYS_Limit']
        fig.add_trace(go.Scatter(x=df['Station'][exceed_mask], y=df['Local_P'][exceed_mask], fill=None, mode='lines', line=dict(color='red', width=0), showlegend=False, yaxis='y2'))
        fig.add_trace(go.Scatter(x=df['Station'][exceed_mask], y=df['SMYS_Limit'][exceed_mask], fill='tonexty', mode='lines', fillcolor='rgba(255,0,0,0.3)', line=dict(color='red', width=0), name='Exceeds Limit', yaxis='y2'))
        ...
        if not df.empty:
            ...
            if not closest_test.empty:
                ...
                fig.add_vline(x=test_sta, line=dict(color='blue', dash='dash'), annotation_text=f'Test Site (Elev {test_elev:.1f} ft)', annotation_position='top right')

        high_row = df.loc[df['Elevation'].idxmax()]
        low_row = df.loc[df['Elevation'].idxmin()]
        fig.add_annotation(x=high_row['Station'], y=high_row['Elevation'], text=f"High: Elev {high_row['Elevation']:.1f} ft", showarrow=True, arrowhead=1, yref='y1')
        fig.add_annotation(x=low_row['Station'], y=low_row['Elevation'], text=f"Low: Elev {low_row['Elevation']:.1f} ft", showarrow=True, arrowhead=1, yref='y1')

        plot1_html = fig.to_html(full_html=False, include_plotlyjs='cdn')

        # Second plot for backpressure
        fig_bp = go.Figure()

        fig_bp.add_trace(go.Scatter(x=df['Station'], y=df['Elevation'], name='Elevation (ft)', line=dict(color='#101820'), yaxis='y1', customdata=df['Station_Formatted'], hovertemplate='Station: %{customdata}<br>Elevation: %{y:.1f} ft<extra></extra>'))

        if 'Req_Back_P' in df.columns:
            fig_bp.add_trace(go.Scatter(x=df['Station'], y=df['Req_Back_P'], name='Required Back Pressure (psig)', line=dict(color='orange'), yaxis='y2', customdata=df['Station_Formatted'], hovertemplate='Station: %{customdata}<br>Req Back P: %{y:.0f} psig<extra></extra>'))

        if 'Prepack_Profile' in df.columns:
            prepack_name = 'Prepack Pressure Profile'
            if params and params.get('override_prepack') is not None:
                prepack_name = 'Prepack Pressure Profile (Override)'
            fig_bp.add_trace(go.Scatter(x=df['Station'], y=df['Prepack_Profile'], name=prepack_name, line=dict(color='green'), yaxis='y2', customdata=df['Station_Formatted'], hovertemplate='Station: %{customdata}<br>Prepack Profile: %{y:.0f} psig<extra></extra>'))

        # Red shading where prepack is below required back pressure
        if 'Prepack_Profile' in df.columns and 'Req_Back_P' in df.columns:
            below_mask = df['Prepack_Profile'] < df['Req_Back_P']
            if below_mask.any():
                fig_bp.add_trace(go.Scatter(x=df['Station'][below_mask], y=df['Prepack_Profile'][below_mask], fill=None, mode='lines', line=dict(color='red', width=0), showlegend=False, yaxis='y2'))
                fig_bp.add_trace(go.Scatter(x=df['Station'][below_mask], y=df['Req_Back_P'][below_mask], fill='tonexty', mode='lines', fillcolor='rgba(255,0,0,0.3)', line=dict(color='red', width=0), name='Below Required', yaxis='y2'))
        ...
        fig_bp.update_xaxes(tickvals=tickvals, ticktext=ticktext)

        plot2_html = fig_bp.to_html(full_html=False, include_plotlyjs='cdn')

        return plot1_html, plot2_html

class Section:
    def __init__(self, app, params, col_map):
        if 'start' not in params or 'end' not in params or 'min_p' not in params or 'test_site' not in params:
        ...
        data['Station'] = data[col_map['station']]
        data['Elevation'] = data[col_map['elev']]
        data['WT'] = data[col_map['wt']]
        self.points = data
